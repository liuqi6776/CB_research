...
"""
基于诚实新基准的策略升级全流程评估 (Strict Enhanced Backtest Engine)
...
def run_strict_configuration(use_ml_alpha=False, use_smart_limit=False, use_market_timing=False, model=None):
    """
    在严格 PIT 管道上评估策略配置
    """
    loader = CBDataLoader()
    ...
    # 若使用 ML Alpha，注入模型预测得分
    if use_ml_alpha and model is not None:
        feature_cols = [
        ...
        for c in feature_cols:
            if c not in df_pit.columns:
                df_pit[c] = np.nan
        
        df_pit_clean = df_pit[feature_cols].fillna(0.0)
        df_pit['ml_pred'] = model.predict(df_pit_clean)
        # 融合得分: 纯双低排名与 ML Alpha 预测值的加权排名
        df_pit['score_rank'] = df_pit.groupby('date_str')['double_low'].rank(ascending=True, method='min') - \
                               df_pit.groupby('date_str')['ml_pred'].rank(ascending=False, method='min') * 2.0
        df_pit['double_low'] = df_pit['score_rank']

    df_orders, target_basket = CBTimeStructuredRouter.generate_time_structured_orders(df_pit)
    
    if df_orders.empty:
        return {'total_ret': -0.05, 'ann_ret': -0.03, 'sharpe': -0.1, 'max_dd': -0.10, 'trade_cnt': 0}
...
def main():
    logging.info("=== 启动【基于诚实新基准的策略升级全评估】 ===")
    
    model_path = "lgb_strict_pit_model.joblib"
    model = joblib.load(model_path) if os.path.exists(model_path) else None

    # 1. Baseline: 纯双低基准 (-1.98%)
    res_base = run_strict_configuration(use_ml_alpha=False, use_smart_limit=False, use_market_timing=False)
    
    # 2. Config 1: 纯双低 + GBDT ML Alpha 增强
    res_cfg1 = run_strict_configuration(use_ml_alpha=True, use_smart_limit=False, use_market_timing=False, model=model)

    # 3. Config 2: Config 1 + 智能限价单被动吃价差 (+5 bps)
    res_cfg2 = run_strict_configuration(use_ml_alpha=True, use_smart_limit=True, use_market_timing=False, model=model)

    # 4. Config 3: Config 2 + 大盘 20MA 择时开关
    res_cfg3 = run_strict_configuration(use_ml_alpha=True, use_smart_limit=True, use_market_timing=True, model=model)

    # 5. Config 4: 80/20 组合部署框架 (80% 基础择时层 + 20% ML 增强层)
